{
  config,
  ...
}:
{
  users.users."${config.hostSpecific.defaultUser.name}" = {
    isNormalUser = true;
    uid = 1000;
    group = "wheel";
    linger = config.hostSpecific.defaultUser.linger;
    extraGroups = [
      "${config.hostSpecific.defaultUser.name}"
      "users"
      "adbusers"
      "audio"
    ] ++ config.hostSpecific.defaultUser.extraGroups;
    openssh.authorizedKeys.keys = [
      "ssh-rsa  wsdlly02@WSdlly02-PC"
    ];
  };
  environment = {
    homeBinInPath = true;
    localBinInPath = true;
  };
}
